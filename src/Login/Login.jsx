    ...
    const [loading,setLoading] = useState(false)
     ...
     async function Login() {
      const user = {
       ...
       try{
        setLoading(true);
        const response = await axios.post('/api/hrm/logins',user);
    ...
    <ThemeProvider  theme={defaultTheme} >
      ...
      <h1 align='center'>ADMIN LOGIN</h1>
       <TextField  margin="normal" required fullWidth id="HR ID" label="HR ID" name="id" 
                autoComplete="id"  autoFocus  onChange={(e) => {setHrId(e.target.value)}} value={hrid}/><br/>

       
      <TextField  margin="normal" required fullWidth name="password" label="Password" type="password" id="password" 
                autoComplete="current-password"   onChange={(e)=>{setPassword(e.target.value)}} value={password}/><br/>
